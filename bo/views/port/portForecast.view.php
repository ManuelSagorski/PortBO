...
$forecast = Forecast::getForecastForPort($port->getID());
$arrivingDay = "";
?>

<form id="addForecast<?php echo $port->getID(); ?>" class="addForecast">
...
?>
    		<tr<?php echo ($expectedVessel->getStatus() == 1)?' class="forecastDisabled"':'';?>>
				...
				<?php echo (!empty($expectedVessel->vessel))?'
                    <a onClick="vessel.openDetails(' . $expectedVessel->vessel->getID() . '); portC.safeForecastPosition();">
    			...
    			<td data-label="createSchedule" class="center aligned collapsing">
    				<?php if (!empty($expectedVessel->vessel)) { ?>
    				<a onClick="vessel.createScheduledContact(<?php echo $expectedVessel->vessel->getID(); ?>, <?php echo $port->getID(); ?>, '<?php echo $tmpDay->format('Y-m-d'); ?>'); portC.safeForecastPosition();"><i class="calendar plus outline icon"></i></a>
    				<?php } ?>
    			</td>
...
<?php } ?>
			<tr>
				...
				<td colspan="2">
					<input type="hidden" name="portID" value="<?php echo $port->getID(); ?>">
				</td>
