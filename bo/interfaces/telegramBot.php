...
if (isset($telegramResponse['callback_query'])) {
    Telegram::manageCallback($telegramResponse['callback_query']);
} else {
    $telegram = new Telegram(null, $telegramResponse);
    Logger::writeLogInfo('telegramBot', 'Eingehende Nachricht: ' . $telegram->getMessage());

    $userTelegram = User::getSingleObjectByCondition(Array(
        "telegram_id" => $telegram->getChatID()
    ));
    ...
    if (empty($userTelegram)) {
        ...
        if (! isset($sent) && ! User::isBoUserActive()) {
            $telegram->sendMessage(false, "Hallo " . $userTelegram->getFirstName() . ". Derzeit ist kein Back-Office Mitarbeiter online. Wir werden dir so schnell wie möglich antworten. " . "Um eine Übersicht der möglichen Befehle zu erhalten, sende bitte '/hilfe'.");
        }
    }
}
?>